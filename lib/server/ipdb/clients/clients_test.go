...
func TestClients(t *testing.T) {
	...
	if err := c.Inject(now, uip.Uip(1), d.Duid{0x01}, leaseLong); err != nil {
	...
	if a, b := c.Lookup(then, uip.Uip(9), d.Duid{0x1}); a != nil || b == nil {
	...
	if a, b := c.Lookup(now, uip.Uip(9), d.Duid{0x1}); a != nil || b == nil {
...
func TestPermanent(t *testing.T) {
	...
	if err := c.InjectPermanent(now, uip.Uip(9), d.Duid{0x99}); err != nil {
	...
	if err := c.Inject(then, uip.Uip(9), d.Duid{0x99}, leaseShort); err == nil {
		t.Errorf("Injecting permanent but expired client returned nil, wanted non-nil.")
	}

	// Lookup at 'now' should work.
	if a, b := c.Lookup(now, uip.Uip(9), d.Duid{0x99}); a == nil || b == nil || a != b {
	...
	if a, b := c.Lookup(then, uip.Uip(9), d.Duid{0x99}); a == nil || b == nil || a != b {
		t.Errorf("Expected to find permanent client, got a=%p, b=%p", a, b)
	}
}
...
func TestLeaseSet(t *testing.T) {
	...
	if err := c.SetLease(now, uip.Uip(9), d.Duid{0x99}, leaseLong); err != nil {
		t.Errorf("SetLease failed with %v; wanted nil.", err)
	}
	// Test wrong params
	if err := c.SetLease(now, uip.Uip(9), d.Duid{0x91}, leaseLong); err == nil {
		t.Errorf("SetLease with invalid hwaddr worked, wanted err")
	}
	if err := c.SetLease(now, uip.Uip(8), d.Duid{0x99}, leaseLong); err == nil {
		t.Errorf("SetLease with invalid ip worked, wanted err")
	}
	if err := c.SetLease(now, uip.Uip(8), d.Duid{0x91}, leaseLong); err == nil {
		t.Errorf("SetLease with invalid ip and hwaddr worked, wanted err")
	}
	// Now lookup at later time.
	if a, b := c.Lookup(then, uip.Uip(9), d.Duid{0x99}); a == nil || a != b {
		t.Errorf("Lookup of extended lease failed")
	}
	// ..and expire it.
	if err := c.Expire(now, uip.Uip(9), d.Duid{0x99}); err != nil {
		t.Errorf("Expire failed")
	}
	// That should now fail
	if a, b := c.Lookup(then, uip.Uip(9), d.Duid{0x99}); a != nil || b != nil {
